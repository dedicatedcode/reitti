...
public class LocationDataApiController {
    ...
    public ResponseEntity<?> receiveLocationData(
        ...
        // Override the userId in the request with the authenticated user
        // This ensures that users can only submit data for themselves
        String originalUserId = request.getUserId();
        if (!user.getUsername().equals(originalUserId)) {
            logger.warn("User {} attempted to submit data for user {}", user.getUsername(), originalUserId);
            // We'll continue processing but use the authenticated user's ID
        }
    ...
    public ResponseEntity<?> receiveLocationPoint(
            @RequestHeader("X-API-Token") String apiToken,
            @Valid @RequestBody LocationDataRequest.LocationPoint point) {
        ...
        try {
            // Process the single location point
            RawLocationPoint savedPoint = locationDataService.processSingleLocationPoint(user, point);
            
            logger.info("Successfully processed location point for user {}", user.getUsername());
            
            return ResponseEntity.ok(Map.of(
                    "success", true,
                    "message", "Successfully processed location point",
                    "pointId", savedPoint.getId()
            ));
            
        } catch (Exception e) {
            logger.error("Error processing location point", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("error", "Error processing location point: " + e.getMessage()));
        }
    }
}
package com.dedicatedcode.reitti.controller.api;

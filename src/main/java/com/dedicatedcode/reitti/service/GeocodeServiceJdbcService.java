...
public class GeocodeServiceJdbcService {
    ...
    public List<GeocodeService> findByEnabledTrueOrderByLastUsedAsc() {
        String sql = "SELECT id, name, url_template, enabled, error_count, last_used, last_error " +
    ...
    public GeocodeService save(GeocodeService geocodeService) {
        if (geocodeService.getId() == null) {
            // Insert new record
            String sql = "INSERT INTO geocode_services (name, url_template, enabled, error_count, last_used, last_error) " +
                        "VALUES (?, ?, ?, ?, ?, ?) RETURNING id";
            ...
            return new GeocodeService(id, geocodeService.getName(), geocodeService.getUrlTemplate(),
                geocodeService.isEnabled(), geocodeService.getErrorCount(), geocodeService.getLastUsed(), geocodeService.getLastError());
        } else {
            // Update existing record
            String sql = "UPDATE geocode_services SET name = ?, url_template = ?, enabled = ?, error_count = ?, last_used = ?, last_error = ? WHERE id = ?";
    ...
    public List<GeocodeService> findAll() {
        String sql = "SELECT id, name, url_template, enabled, error_count, last_used, last_error FROM geocode_services";
        return jdbcTemplate.query(sql, GEOCODE_SERVICE_ROW_MAPPER);
    }

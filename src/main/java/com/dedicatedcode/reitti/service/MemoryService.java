    ...
    public List<MemoryBlockPart> getBlockPartsForMemory(Long memoryId) {
        ...
        for (MemoryBlock block : blocks) {
            switch (block.getBlockType()) {
                case TEXT:
                    memoryBlockJdbcService.findTextBlock(block.getId())
                        .ifPresent(blockParts::add);
                    break;
                case VISIT:
                    memoryBlockJdbcService.findVisitBlock(block.getId())
                        .ifPresent(blockParts::add);
                    break;
                case TRIP:
                    memoryBlockJdbcService.findTripBlock(block.getId())
                        .ifPresent(blockParts::add);
                    break;
                case IMAGE_GALLERY:
                    List<MemoryBlockImageGallery> galleryImages = memoryBlockJdbcService.findImageGalleryBlocks(block.getId());
    ...
    public void recalculateMemory(User user, Long memoryId) {
        ...
        for (MemoryBlockPart autoGeneratedBlock : autoGeneratedBlocks) {
            if (autoGeneratedBlock instanceof MemoryBlockText textBlock) {
                ...
                memoryBlockJdbcService.createTextBlock(memoryBlock.getId(), textBlock);
                ...
                memoryBlockJdbcService.createImageGalleryBlock(memoryBlock.getId(), imageGalleryBlock);
            } else if (autoGeneratedBlock instanceof MemoryBlockTrip tripBlock) {
                MemoryBlock memoryBlock = addBlock(memoryId, BlockType.TRIP);
                memoryBlockJdbcService.createTripBlock(memoryBlock.getId(), tripBlock);
                ...
                memoryBlockJdbcService.createVisitBlock(memoryBlock.getId(), visitBlock);
            }
        }

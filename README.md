...
| `TILE_PROXY_SIGNATURE_SECRET`  | Signing secret for proxied tile URLs. Set explicitly to share sessions across instances and persist them across restarts - otherwise regenerated per instance restart.                      |                     | SOME__EXAMPLE_SECRET_123 |
| `PROXY_LOCAL_TILE_URLS`        | Allow the backend/tile-cache proxy to fetch local network tile URLs. Disable unless styles need proxy access to the local network; when disabled, browsers fetch local URLs directly. | false               | true                             |
